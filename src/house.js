...
function getThisArticle() {
  let thisPostId = Number(new URL(document.location).searchParams.get('id'));
  ...
  const firebaseUser = database.ref(`member/${thisUserId}`);
...
function createSelectorImg() {
  ...
  for (let key in thisData[0].houseImg) {
  ...
  const thisImg = app.get('#displayHouseImg').getAttribute('data-img');
...
function showThisImg(e) {
  ...
  const thisImg = e.target.getAttribute('data-no');
...
function createHouseData() {
  ...
  for (const kindType in thisData[0].require) {
    app.createElement('div', `houseRequireRow${count}`, 'house_require_row', 'houseRequireContainer', '', '');
    app.createElement('p', `houseRequireTitle${count}`, 'house_require_title', `houseRequireRow${count}`, kindType, '');
    app.createElement('p', '', 'house_separate', `houseRequireRow${count}`, ' : ', '');
    app.createElement('p', '', 'house_require_detail', `houseRequireRow${count}`, thisData[0].require[kindType], '');
    count += 1;
  }

  /* create device */
  app.createElement('p', '', 'house_device_title', 'houseDeviceContainer', '房東提供', '');
  for (let key in thisData[0].device) {
...
function createHouseInfo() {
  ...
  let priceInclude = '含';
  for (let key in thisData[0].priceInclude) {
...
function createOwnerInfo() {
  ...
  const userName = getUser.userName.substr(0, 1) + gender;
