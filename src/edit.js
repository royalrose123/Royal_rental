...
app.get('#houseEditConfirm').addEventListener('click', () => {
  let address = regionName.value + sectionName.value + streetName.value;
  addressToLatLng(address);

  deleteImgArr = deleteImgArr.filter((element, index, arr) => arr.indexOf(element) === index);
  deleteFirebaseImg = thisDataImgArr.concat(deleteImgArr);
  deleteFirebaseImg = deleteFirebaseImg.filter((element, index, arr) => arr.indexOf(element) !== index);
